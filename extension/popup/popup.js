          ...
          if (mediaCard && mediaList) {
            ...
            rawStreams.forEach((streamUrl) => {
              ...
              if (streamUrl.includes(".mpd")) {
                label = "DASH Full Video Stream (.mpd)";
                badge = "DASH";
                priority = 1;
              } else if (streamUrl.includes(".m3u8")) {
                label = "HLS Full Video Stream (.m3u8)";
                badge = "HLS";
                priority = 2;
              } else if (streamUrl.includes(".mp4")) {
                label = "Full MP4 Video (Original)";
                badge = "MP4";
                priority = 3;
              } else if (streamUrl.includes(".webm")) {
                label = "Full WebM Video";
                badge = "WEBM";
                priority = 4;
              ...
              validStreams.push({ url: streamUrl, label: label, badge: badge, priority: priority });
            });
            ...
            validStreams.forEach((item) => {
              ...
              row.title = item.url;
              row.innerHTML = `<span class="media-item-title">${item.label}</span><span class="media-item-badge">${item.badge}</span>`;
